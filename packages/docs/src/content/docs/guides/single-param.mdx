...
## Custom defaults with `.default()`

Use `.default(value)` to override a type's inherent default. For example, `presets.integer` defaults to `NaN`, but a page number should default to `1`:
...
`.default()` also works with rounding variants and float precision:

```ts
presets.integer.ceil.default(0); // Math.ceil, default 0
