...
function zodEnum<T extends [string, ...string[]]>(schema: z.ZodEnum<T>) {
  const options = schema.options;
  return createPreset({
    decode: (v) => {
      const result = schema.safeParse(v);
      if (!result.success) throw new Error("invalid enum value");
      return result.data;
    },
...
const statusStore = qsUtils.createSearchParamStore("status", zodEnum(StatusSchema)());
const optionalStatus = qsUtils.createSearchParamStore(
  "status",
  zodEnum(StatusSchema)({ optional: true }),
);
```
